function result = run_pipeline(observations, k, min_obs, min_bads, prior, increasing, min_confidence, use_counts)
...
%   result = RUN_PIPELINE(observations, k, min_obs, min_bads, prior, increasing, min_confidence, use_counts)
%
%   Chains:
...
%                    An optional `weight` column (or 3rd matrix column) enables
%                    value-weighted binning.
...
%   use_counts     — logical (default true); forwarded to enforce_minimum_size.
%
...
if nargin < 8 || isempty(use_counts);      use_counts     = true;  end
