...
# infra/README.md). Creates the Terraform remote state backend (S3 +
# DynamoDB) and the GitHub Actions OIDC deploy role used by infra/.
#
# The GitHub Actions OIDC identity provider is account-wide and was
# already created by another project's bootstrap, so it is looked up
# here rather than created again.
